...
func _process(d):
	$ViewportContainer/Viewport/Camera.global_transform =  $Yaw/Camera.global_transform
	$RigViewportContainer/Viewport/Camera.global_transform =  $Yaw/Camera.global_transform
#######################################################################################################
# INTERACTIONS
...
func _on_HighQuality_body_entered(body):
	if body.has_method("set_quality_high"):
		body.set_quality_high()


func _on_HighQuality_body_exited(body):
	if body.has_method("set_quality_low"):
		body.set_quality_low()


func _on_Proximity_body_entered(body):
	if body.has_method("set_quality_proximity"):
		body.set_quality_proximity()
